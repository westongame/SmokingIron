...
public class GunItem extends Item implements IColored, IMeta
{
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag flag)
    {
        ...
        if(ammo != null)
        {
            tooltip.add(Component.translatable("info.smoking_iron.ammo_type", Component.translatable(ammo.getDescriptionId()).withStyle(ChatFormatting.WHITE)).withStyle(ChatFormatting.GRAY));
        }
        ...
        tooltip.add(Component.translatable("info.smoking_iron.damage", ChatFormatting.WHITE + ATTRIBUTE_FORMAT.format(damage) + additionalDamageText).withStyle(ChatFormatting.GRAY));

        if(stack.has(ModDataComponents.IGNORE_AMMO.get()))
        {
            tooltip.add(Component.translatable("info.smoking_iron.ignore_ammo").withStyle(ChatFormatting.AQUA));
        }
        else
        {
            int ammoCount = stack.getOrDefault(ModDataComponents.AMMO_COUNT.get(), 0);
            tooltip.add(Component.translatable("info.smoking_iron.ammo", ChatFormatting.WHITE.toString() + ammoCount + "/" + GunEnchantmentHelper.getAmmoCapacity(stack, modifiedGun)).withStyle(ChatFormatting.GRAY));
        }

        tooltip.add(Component.translatable("info.smoking_iron.attachment_help", KeyBinds.KEY_ATTACHMENTS.getTranslatedKeyMessage().getString().toUpperCase(Locale.ENGLISH)).withStyle(ChatFormatting.YELLOW));
    }
