package com.westongame.smokingiron.client.screen;
...
import com.westongame.smokingiron.client.screen.widget.MiniButton;
...
import com.westongame.smokingiron.common.container.slot.AttachmentSlot;
...
public class AttachmentScreen extends AbstractContainerScreen<AttachmentContainer> {
    private static final ResourceLocation GUI_TEXTURES = ResourceLocation.parse("smoking_iron:textures/gui/attachments.png");
    private static final Component CONFIG_TOOLTIP = Component.translatable("smoking_iron.button.config.tooltip");
        ...
        for (int i = 0; i < IAttachment.Type.values().length; i++) {
            if (RenderUtil.isMouseWithin(mouseX, mouseY, startX + 7, startY + 16 + i * 18, 18, 18)) {
                ...
                if (!this.menu.getSlot(i).isActive()) {
                    graphics.renderComponentTooltip(this.font, Arrays.asList(Component.translatable("slot.smoking_iron.attachment." + type.getTranslationKey()), Component.translatable("slot.smoking_iron.attachment.not_applicable")), mouseX, mouseY);
                    ...
                    graphics.renderComponentTooltip(this.font, Arrays.asList(Component.translatable("slot.smoking_iron.attachment.incompatible").withStyle(ChatFormatting.YELLOW)), mouseX, mouseY);
                } else if (this.weaponInventory.getItem(i).isEmpty()) {
                    graphics.renderComponentTooltip(this.font, Collections.singletonList(Component.translatable("slot.smoking_iron.attachment." + type.getTranslationKey())), mouseX, mouseY);
                }
            }
        }
    }
    ...
    protected void renderLabels(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (this.showHelp) {
            ...
            graphics.drawString(minecraft.font, I18n.get("container.smoking_iron.attachments.window_help"), 56, 38, 0xFFFFFF, false);
            graphics.pose().popPose();
        }
    }
    ...
    private void openConfigScreen() {
        ModList.get().getModContainerById(Reference.MOD_ID).ifPresent(container -> {
            ...
            if (screen != null) {
                ...
                modName.setStyle(modName.getStyle().withColor(ChatFormatting.YELLOW).withUnderlined(true).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("smoking_iron.chat.open_curseforge_page"))).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://www.curseforge.com/minecraft/mc-mods/configured")));
                Component message = Component.translatable("smoking_iron.chat.install_configured", modName);
